...
@EActivity
public class MeasureActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
...
//添加转点
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lineExtra = (LineExtra) getIntent().getSerializableExtra("line_extra");//lineExtra
        totalMeasureCount = lineExtra.getMtype().contains("BFFB") ? 4 : 2;
        lineinfoMap = (Map<String, Object>) getIntent().getSerializableExtra("lineinfo");//line &&bw
    ...
    public void onItemClick(AdapterView<?> parent, View view,
        ...
        //充值测量计数
        measureCounterForStation = 0;//开始计数测量次数
...
private void fillMeasure(String mtype, int order, double hd, double r){
    ...
    private void lineStation2ListData(List<LineStation> lineStations) {
        ...
        List<LineStation> lineStations_ori = new ArrayList<LineStation>();//原始数据
        lineStations_ori.addAll(lineStations);

        List<LineStation> lineStations_dst = new ArrayList<LineStation>();//目标数据
     ...
     * 测站智能排序
     *
     * @param lineStation      测站
     * @param lineStations_ori 原始数据
     * @param lineStations_dst 目标数据
     * @return
     */
    private LineStation searchByBpnt(LineStation lineStation, List<LineStation> lineStations_ori, List<LineStation> lineStations_dst) {
        ...
        if (lineStation != null && lineStations_ori.size() != 0) {
            for (int i = 0; i < lineStations_ori.size(); i++) {
                if (lineStations_ori.get(i).getSb().equalsIgnoreCase(lineStation.getSf())) {//找到测站
                    ...
                    return searchByBpnt(lineStation_result, lineStations_ori, lineStations_dst);//递归调用
                }
            }
        }
